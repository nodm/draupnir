...
- [x] 8.2 Verify a malformed file exercises the DLQ path: upload a deliberately broken file, confirm it lands in the DLQ after `maxReceiveCount` retries and no partial rows exist.
  Verified 2026-09-01 on `prod`: uploaded a SEB file with a truncated row (5 fields instead of 19). The parse failure threw before any write, so the whole invocation failed and SQS redelivered it; after 5 receives it landed in the DLQ (confirmed by receiving and inspecting the message) and `transactions` had 0 rows for that account. Test account and DLQ message removed after.
