...
- [ ] 4.1 Once deployed with real AWS credentials, run `pulumi preview`/`pulumi up` against a test/dev stack and confirm: the cluster reaches `available` with Data API enabled, the security group has no ingress rules, and `import-bank-statements`'s `POST /accounts` and presigned-upload endpoints succeed against the live `DbConfig` (unblocking that change's tasks 8.1/8.2). Not runnable in a sandbox without AWS credentials — do this manually when deploying.
